...
Chained-scenario rules include an `artifacts.dashboards: [{id}, …]` block linking each rule to **the chain overview dashboard plus one or more service-specific dashboards** that match the rule's primary dataset (e.g. AWS S3 mass-access links the chain overview, CloudTrail, **and** S3; Azure full-chain IAM links the chain overview, Entra ID, **and** Activity Log). Multi-source correlation rules (the chain "burst" / "full-chain" rules) deliberately link only the chain overview because that's where the cross-service panels live. When an alert fires, Kibana's **Alert Details** page surfaces all of those under a **Related dashboards** tab. IDs are deterministic UUIDs derived from dashboard titles (same algorithm as the wizard installer) — install the matching dashboards via the same asset bundle and the links resolve automatically. Requires Kibana **8.19 / 9.1+**; older versions ignore the field. The full per-rule mapping is in [docs/SETUP-WIZARD-AND-UNINSTALL.md → Linked dashboards on alerts](../docs/SETUP-WIZARD-AND-UNINSTALL.md#linked-dashboards-on-alerts).

Per-rule **investigation guides** for all 51 chained-scenario alerts are in [docs/runbooks/](../docs/runbooks/) — five-minute triage, ES|QL queries, likely causes, containment / remediation, and escalation criteria. The wizard, `setup:alert-rules` CLI, and the alert-enrichment workflow's email body all surface the runbook URL after install or on every notification.
