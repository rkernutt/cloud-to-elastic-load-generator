...
The Setup wizard also installs **SLO definitions** (availability and data-pipeline SLIs per cloud) via the Kibana Observability SLO API, and optional **Agent Builder** tool definitions for AI-assisted investigation — including a dedicated **SOC Analyst** agent with 7 security-focused tools for investigating attack chains, querying CMDB context, and triaging security alerts.

## AI SOC demo

Cloud Loadgen ships a complete AI SOC demo scenario built around **IAM privilege escalation**, with **16 Elastic Security detection rules** (6 IAM, 6 finding, 4 exfiltration) that produce 50+ alerts for **Attack Discovery**, a **security alert enrichment workflow** that adds originating IP and hostname from **ServiceNow CMDB**, and an **Agent Builder SOC Analyst** for conversational investigation. Full walkthrough in **[docs/SOC-DEMO-SETUP.md](docs/SOC-DEMO-SETUP.md)**.
