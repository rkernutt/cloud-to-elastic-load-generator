...
import { AZURE_METRICS_GENERATORS } from "./metrics/index.js";
...
describe("Azure generator smoke", () => {
  it("every log generator returns an ECS-shaped root with @timestamp", () => {
    for (const id of Object.keys(AZURE_GENERATORS)) {
      const gen = AZURE_GENERATORS[id]!;
      const raw = gen(TS, ER);
  ...
  it("every metric generator returns non-empty @timestamp metric rows", () => {
    for (const id of Object.keys(AZURE_METRICS_GENERATORS)) {
      const gen = AZURE_METRICS_GENERATORS[id]!;
      const raw = gen(TS, ER);
