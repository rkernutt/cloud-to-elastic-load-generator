...
export function generateTimeSeriesInsightsLog(ts: string, er: number): EcsDocument {
  ...
  const correlationId = randUUID();
  const time = azureDiagnosticTime(ts);
  const variant = rand(["ingest", "query", "admin", "warmPath", "export", "capacity"] as const);
  ...
  if (variant === "ingest") {
    const props = {
      ...
      ...integrationExtendedErrFields(
        isErr,
        "TSI ingress pipeline lagged or dropped events from Event Hub source",
        "data"
      ),
    };
    return {
  ...
  if (variant === "query") {
    const props = {
      ...
      queryIssue: isErr ? "partial result: shard timeout" : "",
      ...integrationExtendedErrFields(
        isErr,
        "TSI query execution returned partial data or shard timeout exceeded",
        "data"
      ),
    };
    return {
  ...
  if (variant === "warmPath") {
    const props = {
      cacheTier: rand(["hot", "warm"]),
      blobsHydrated: isErr ? randInt(0, 40) : randInt(80, 5000),
      secondsBehind: isErr ? randFloat(600, 7200) : randFloat(0.5, 180),
      ...integrationExtendedErrFields(
        isErr,
        "Warm storage path fell behind SLA while hydrating historical intervals",
        "data"
      ),
    };
    return {
      ...
      time,
      resourceId,
      operationName: "Microsoft.TimeSeriesInsights/environments/warmStore/sync",
      category: "WarmStorage",
      resultType: isErr ? "Failure" : "Success",
      resultSignature: isErr ? "stale" : "current",
      callerIpAddress: callerIp,
      correlationId,
      level: isErr ? "Warning" : "Information",
      properties: props,
  ...
  if (variant === "export") {
    const props = {
      sink: rand(["blobAccount", "dataLake"]),
      parquetFiles: isErr ? 0 : randInt(1, 800),
      bytesExported: isErr ? randInt(0, 1e8) : randInt(5e9, 4e11),
      ...integrationExtendedErrFields(
        isErr,
        "Bulk export job failed writing Parquet batches to configured sink",
        "data"
      ),
    };
    return {
      ...
      time,
      resourceId,
      operationName: "Microsoft.TimeSeriesInsights/environments/data/export",
      category: "DataExport",
      resultType: isErr ? "Failure" : "Success",
      resultSignature: isErr ? "aborted" : "committed",
      callerIpAddress: callerIp,
      correlationId,
      level: isErr ? "Error" : "Information",
      properties: props,
  ...
  if (variant === "capacity") {
    const props = {
      skuCapacityUnits: randInt(1, 10),
      utilizationPct: isErr ? randFloat(92, 100) : randFloat(12, 78),
      throttledRequests: isErr ? randInt(120, 9000) : randInt(0, 40),
      ...integrationExtendedErrFields(
        isErr,
        "TSI environment hit capacity throttle on ingress or query plane",
        "data"
      ),
    };
    return {
      ...
      time,
      resourceId,
      operationName: "Microsoft.TimeSeriesInsights/environments/capacity/read",
      category: "CapacityPlanning",
      resultType: isErr ? "Failure" : "Success",
      resultSignature: isErr ? "saturated" : "headroom",
      callerIpAddress: callerIp,
      correlationId,
      level: isErr ? "Warning" : "Information",
      properties: props,
  ...
  const props = {
    ...
    ...integrationExtendedErrFields(
      isErr,
      "TSI environment ARM write failed validation or SKU change blocked",
      "adminOrProvision"
    ),
  };
  return {
