...
const WORKFLOW_IDS = [
  ...
  "workflow-data",
  "workflow-cascading",
  "workflow-pubsub-fanout",
  "workflow-gcs-pipeline",
  "workflow-eventarc-orchestration",
];
...
describe("All GCP_TRACE_GENERATORS — structural invariants", () => {
...
describe("GCP workflow generators — error branch at er=1", () => {
  for (const name of WORKFLOW_IDS) {
    it(`${name}: errorRate=1 produces at least one failure outcome`, () => {
      const gen = GCP_TRACE_GENERATORS[name];
      expect(gen, `${name} not in registry`).toBeDefined();

      let hasFailure = false;
      for (let attempt = 0; attempt < 10; attempt++) {
        const docs = gen!(TS, 1) as Doc[];
        if (docs.some((d) => (d.event as Record<string, unknown>)?.outcome === "failure")) {
          hasFailure = true;
          break;
        }
      }
