...
export function generateComputeEngineLog(ts: string, er: number): EcsDocument {
  ...
  const status = isErr ? rand(["ERROR", "FAILED", "TERMINATED"]) : rand(["RUNNING", "STAGING", "DONE", "OK"]);
...
export function generateBatchLog(ts: string, er: number): EcsDocument {
  ...
  const status = isErr ? rand(["FAILED", "FAILED", "QUEUED"]) : rand(["QUEUED", "RUNNING", "SUCCEEDED"]);
...
export function generateSoleTenantNodesLog(ts: string, er: number): EcsDocument {
  ...
  const status = isErr ? rand(["ERROR", "MAINTENANCE_FAILED"]) : rand(["READY", "PROVISIONING", "RUNNING"]);
...
export function generateVmwareEngineLog(ts: string, er: number): EcsDocument {
  ...
  const eventType = isErr
    ? rand(["VMWARE_CLUSTER_CREATING_FAILED", "NODE_ADDED_FAILED", "MAINTENANCE_DEFERRED", "STRETCHED_NETWORK_ERROR"])
    : rand(["VMWARE_CLUSTER_CREATING", "NODE_ADDED", "MAINTENANCE", "UPGRADE_COMPLETED", "NSXT_CONFIG_UPDATED"]);
...
export function generateBareMetalLog(ts: string, er: number): EcsDocument {
  ...
  const machineType = rand(["o2-standard-16-metal", "m3-megamem-128-metal", "n2-standard-80-metal"]);
...
export function generateSpotVmsLog(ts: string, er: number): EcsDocument {
  ...
  const preemptionReason = isErr
    ? rand(["SPOT_PREEMPTED", "INSTANCE_TERMINATED_BY_SERVICE", "MAINTENANCE_EVENT", "RESOURCE_UNAVAILABLE"])
    : null;
...
export function generateCloudTpuLog(ts: string, er: number): EcsDocument {
  ...
  const status = isErr ? rand(["ERROR", "FAILED"] as const) : rand(["READY", "CREATING", "RUNNING"] as const);
  const acceleratorCount = randInt(1, 8);
  const utilizationPct = isErr ? randFloat(0.05, 0.35) : randFloat(0.4, 0.98);
  const healthState = isErr ? rand(["UNHEALTHY", "DEGRADED"] as const) : rand(["HEALTHY", "OK"] as const);
  const message = isErr
...
export function generateCloudWorkstationsLog(ts: string, er: number): EcsDocument {
  ...
  const action = isErr ? rand(["STOP", "DELETE"] as const) : rand(["START", "STOP", "CREATE", "DELETE"] as const);
  const machineType = rand(["e2-standard-4", "n1-standard-8", "n1-highgpu-4"]);
  const idleTimeoutMin = randInt(30, 480);
  const userEmail = rand([`dev@${project.id.split("-")[0]}.example.com`, `engineer@${project.id}.example.com`]);
  const message = isErr
...
export function generateShieldedVmsLog(ts: string, er: number): EcsDocument {
  ...
  const integrityStatus = isErr ? rand(["VIOLATED", "UNKNOWN"] as const) : rand(["PASS", "TRUSTED", "VERIFIED"] as const);
  const policyUpdate = isErr ? rand(["UEFI_VARS_MODIFIED", "BOOT_POLICY_MISMATCH"] as const) : rand(["NONE", "REFRESHED", "COMPLIANT"] as const);
  const message = isErr
...
export function generateConfidentialComputingLog(ts: string, er: number): EcsDocument {
  ...
  const attestationStatus = isErr ? rand(["FAILED", "REJECTED"] as const) : rand(["VERIFIED", "PENDING", "ACCEPTED"] as const);
...
export function generateMigrateToVmsLog(ts: string, er: number): EcsDocument {
  ...
  const migrationStatus = isErr ? rand(["REPLICATING", "IDLE"] as const) : rand(["IDLE", "REPLICATING", "CUTOVER", "COMPLETED"] as const);
