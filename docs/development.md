...
- **A field a dashboard groups or counts by must exist on every document in scope.** ES|QL fails the whole panel with `Unknown column` rather than rendering an empty result, so one service with no `event.action` breaks its KPI outright. The other instance of this rule is the Glue `aws.glue.error_category` sentinel `"NONE"` on successful runs (see [GLUE-METRICS-COVERAGE.md](./GLUE-METRICS-COVERAGE.md)) — the column stays mapped so ES|QL can query it.
- **`event.action` is derived centrally, not per generator.** `src/aws/generators/eventAction.ts` exports `normalizeAction`, `deriveEventAction`, and `applyEventAction`. `src/aws/generators/index.ts` wraps every entry of the `GENERATORS` registry with `applyEventAction`, so the field is filled on every path (preview, ship, exported samples, tests). Generators that already set `event.action` are untouched. Coverage went from 69 of 219 AWS log generators to 129 of 219.
- **The value comes from a field the document already carries.** Verb-like fields are checked first, in order: `event_name`, `eventName`, `operation`, `operation_name`, `operation_type`, `api_call`, `api_name`, `api`, `action`, `action_type`, `command`, `verb`, `request_type`, `request_method`, `method`, `event_type`, `event_subtype`, `event_code`, `activity_type`. A narrow list of subject-like fields is consulted only when a service has no verb: `finding_type`, `behavior_type`, `log_kind`, `log_line_kind`, `message_type`, `notification_type`, `record_type`, `change_type`, `job_phase`, `lifecycle`, `alarm_state`.
- **Generic `type`, `status`, and `state` are deliberately excluded.** They describe the resource or the outcome, not the event: an ALB's `type` is `application` and a job's `status` is `SUCCESS`. Mapping either onto `event.action` produces a field that reads plausibly and means nothing.
- **The native value is taken verbatim** (trimmed, 80 characters maximum) and never re-cased, because Elastic's AWS integrations keep the service's own spelling: CloudTrail's `event.action` is `TerminateInstances`, S3's is `REST.GET.OBJECT`. Real values now produced include s3 → `REST.GET.OBJECT`, vpc → `accept` / `reject`, ecr → `scan` / `pull` / `push`, apigateway → `CONNECT` / `DISCONNECT` / `MESSAGE`, lambda → `start` / `end` / `report`, rds → `Query` / `Disconnect`.
- **It never invents a value.** When the document carries nothing action-like, `deriveEventAction` returns `undefined` and the field is left unset.
- **Dashboards that could not be fixed by the derivation were changed instead.** 63 AWS service dashboards carried the templated KPI `STATS \`Distinct actions\` = COUNT_DISTINCT(\`event.action\`)`. 28 of those services now emit the field and keep the KPI. The remaining 34, which have no action concept at all (acm, datasync, vpn, privatelink, devopsguru, servicequotas, cloudwatch, health, and others), use `STATS \`Distinct regions\` = COUNT_DISTINCT(cloud.region)` with the metric column renamed to "Distinct regions". Panel uids were left unchanged.
- **The same derivation exists in two places, on purpose.** `installer/shared/pipeline-processors.mjs` adds a painless processor tagged `set_event_action_from_service_fields` inside `ecsNorm()`, with the same field lists, the same verbatim rule, and `override: false` semantics (it returns early when `ctx.event.action` is already set). Generated documents never reach it: `src/ship/runShipWorkload.ts` posts them to `_bulk` with `{"create":{"_index":…}}` and no `pipeline` parameter, so the installed custom pipelines only apply to real AWS data the customer routes through them. The pipeline covers real data, the generator wrapper covers generated data. Keep the two lists in step when you change either.
...
| `installer/`                     | CLI installers and asset JSON (497 dashboards, 778 ML jobs, 246 rules, pipelines)                                         |
