...
| Set up OpenLineage run lineage (MWAA + EMR Spark → Elastic Agent HTTP endpoint) and demo it | [chained-events/data-pipeline-lineage.md](./chained-events/data-pipeline-lineage.md)                                                                     |
...
Every Elastic asset the app installs — ingest pipelines, data stream templates, Kibana dashboards, ML anomaly detection jobs, and alerting rules — is bundled **per service** as a Cloud Loadgen Integration and tagged **`cloudloadgen`**. The catalog ships **497 dashboards**, **778 ML jobs**, and **246 alerting rules** across AWS, GCP, Azure, and Supporting Services. That tag makes it easy to view, bulk-edit, or bulk-delete load-generator assets in Kibana without touching production objects. The full behaviour (categories, post-install options, Serverless limits, dashboard fallback, alerting rule compatibility) is in [SETUP-WIZARD-AND-UNINSTALL.md](./SETUP-WIZARD-AND-UNINSTALL.md).
...
| AWS Data & Analytics Pipeline (S3 → EMR → Glue Data Quality → Kafka (Confluent) → S3)   | [chained-events/data-analytics-pipeline.md](./chained-events/data-analytics-pipeline.md)               |
| AWS Data & Analytics Pipeline — run lineage (OpenLineage from Airflow/MWAA + Spark/EMR) | [chained-events/data-pipeline-lineage.md](./chained-events/data-pipeline-lineage.md)                   |
