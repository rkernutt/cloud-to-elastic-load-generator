...
Every Elastic asset the app installs — ingest pipelines, data stream templates, Kibana dashboards, ML anomaly detection jobs, and alerting rules — is bundled **per service** as a Cloud Loadgen Integration and tagged **`cloudloadgen`**. The catalog ships **496 dashboards**, **62 ML jobs**, and **31 alerting rules** across AWS, GCP, Azure, and Supporting Services. That tag makes it easy to view, bulk-edit, or bulk-delete load-generator assets in Kibana without touching production objects. The full behaviour (categories, post-install options, Serverless limits, dashboard fallback, alerting rule compatibility) is in [SETUP-WIZARD-AND-UNINSTALL.md](./SETUP-WIZARD-AND-UNINSTALL.md).
