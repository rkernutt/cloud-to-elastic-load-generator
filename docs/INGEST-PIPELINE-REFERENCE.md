...
| **`ecsNorm`**              | Painless (`set_event_action_from_service_fields`) | Derives ECS `event.action` from the service's own operation field on the `aws.{service}` block. Returns immediately when `ctx.event.action` is already set (`override: false` semantics). Checks verb-like names first (`event_name`, `eventName`, `operation`, `operation_name`, `operation_type`, `api_call`, `api_name`, `api`, `action`, `action_type`, `command`, `verb`, `request_type`, `request_method`, `method`, `event_type`, `event_subtype`, `event_code`, `activity_type`), then a narrow subject list used where a service has no verb (`finding_type`, `behavior_type`, `log_kind`, `log_line_kind`, `message_type`, `notification_type`, `record_type`, `change_type`, `job_phase`, `lifecycle`, `alarm_state`). The value is taken verbatim (trimmed, 80 chars max) — CloudTrail's is `TerminateInstances`, S3's is `REST.GET.OBJECT`. Generic `type` / `status` / `state` are excluded because they describe the resource or the outcome, not the event. |

> **Why this also exists in the generators.** Generated documents are bulk-indexed straight to `_bulk` with no `pipeline` parameter (`src/ship/runShipWorkload.ts` writes `{"create":{"_index":…}}`), so the installed custom pipelines only apply to real AWS data the customer routes through them. `src/aws/generators/eventAction.ts` performs the identical derivation for generated documents. The duplication is deliberate: the pipeline covers real data, the generator wrapper covers generated data.
