...
| ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Data & Analytics Pipeline** | `data-pipeline-*` dashboards, ML jobs, **9** rules on AWS (7 incl. the Glue DQ score-low rule + 2 OpenLineage-based Lineage rules, plus the run-lineage dashboard), **5** rules each on GCP / Azure. AWS S3→EMR→Glue Data Quality→Kafka (Confluent)→S3 · GCP GCS→Dataproc→BigQuery→Composer · Azure Blob→Databricks→Synapse→Data Factory | [chained-events/data-analytics-pipeline.md](./chained-events/data-analytics-pipeline.md) (+ GCP / Azure variants); lineage: [chained-events/data-pipeline-lineage.md](./chained-events/data-pipeline-lineage.md) |
...
Installing **all** chained-scenario rule files for one cloud gives you the full set of chain rules — **20** on AWS (8 + 4 + 4 + 4), **17** on GCP and Azure (5 + 4 + 4 + 4). Beyond these, **per-service domain rules** cover compute, database, networking, AI/ML, storage, messaging, DevOps, and security-ops — **245 rules total** across all clouds (AWS 117, GCP 62, Azure 66). Use `npm run setup:alert-rules` (cross-cloud) or the web-UI Setup step.
