...
| 8   | `notify_email`                   | `email`                                | Sends the enriched alert via `inputs.emailConnector`. The body includes rule name, alert message, affected CI / owner / support group / department, the user who triggered the pipeline (with email / phone / manager), open-incident and recent-change counts from CMDB, plus a link to [`docs/runbooks/`](./runbooks/) so the on-call has the per-rule investigation guide one click away. Retries twice on transient SMTP errors then continues so step 9 still runs (the pre-flight already proved the connector exists). |
