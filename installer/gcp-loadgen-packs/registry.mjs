 ...
 * Asset membership is derived from installer assets and src/gcp/data/elasticMaps.ts.
 *
 * Each pack bundles:
...
import { PIPELINE_REGISTRY } from "../gcp-custom-pipelines/pipelines/registry.mjs";
import { loadDashboards } from "../gcp-custom-dashboards/installLib.mjs";

const __dirname = dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = join(__dirname, "../..");
const ELASTIC_MAPS_TS = join(REPO_ROOT, "src/gcp/data/elasticMaps.ts");
const ML_JOBS_DIR = join(REPO_ROOT, "installer/gcp-custom-ml-jobs/jobs");
const DASHBOARDS_DIR = join(REPO_ROOT, "installer/gcp-custom-dashboards");
...
/**
 * Parse `GCP_ELASTIC_DATASET_MAP` and `GCP_METRICS_DATASET_MAP` object literals from elasticMaps.ts.
 */
export function parseElasticMapsFile(path = ELASTIC_MAPS_TS) {
  ...
  const metricsMap = {};
  let mode = null;
  const keyVal = /^\s*(?:"([a-zA-Z0-9_-]+)"|([a-zA-Z0-9_]+))\s*:\s*"([^"]+)"\s*,?\s*$/;

  for (const line of lines) {
    if (line.includes("const GCP_ELASTIC_DATASET_MAP")) {
      mode = "logs";
      continue;
    }
    ...
    if (line.includes("const GCP_METRICS_DATASET_MAP")) {
      mode = "metrics";
      continue;
    }
    ...
    if (m && mode === "logs") logsMap[m[1] ?? m[2]] = m[3];
    if (m && mode === "metrics") metricsMap[m[1] ?? m[2]] = m[3];
  }
...
export function resolveServiceIdsForDataset(dataset, inv) {
  ...
  if (!dataset.startsWith("gcp.")) return [];
