 ...
 * Asset membership is derived from installer assets and src/azure/data/elasticMaps.ts.
 *
 * Each pack bundles:
 *   - Ingest pipeline(s) — log parsers for logs-azure.* data streams
...
import { PIPELINE_REGISTRY } from "../azure-custom-pipelines/pipelines/registry.mjs";
import { loadDashboards } from "../aws-custom-dashboards/installLib.mjs";

const __dirname = dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = join(__dirname, "../..");
const ELASTIC_MAPS_TS = join(REPO_ROOT, "src/azure/data/elasticMaps.ts");
const ML_JOBS_DIR = join(REPO_ROOT, "installer/azure-custom-ml-jobs/jobs");
const DASHBOARDS_DIR = join(REPO_ROOT, "installer/azure-custom-dashboards");
...
/**
 * Parse `AZURE_ELASTIC_DATASET_MAP` and `AZURE_METRICS_DATASET_MAP` object literals from elasticMaps.ts.
 */
export function parseElasticMapsFile(path = ELASTIC_MAPS_TS) {
  ...
  for (const line of lines) {
    ...
    if (line.includes("const AZURE_METRICS_DATASET_MAP")) {
      mode = "metrics";
      continue;
    }
...
export function resolveServiceIdsForDataset(dataset, inv) {
  ...
  if (!dataset.startsWith("azure.")) return [];

  const tail = dataset.slice(6);
